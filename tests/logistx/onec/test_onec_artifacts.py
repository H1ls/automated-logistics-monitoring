...
class TestOneCArtifacts:
    ...
    def test_uses_standard_stage_and_artifact_filename(self):
        with TemporaryDirectory() as tmp:
            ...
            assert path.name == 'fill_times__cargo_search.png'
            assert session.captures == [('region', 'area', 'fill_times__cargo_search.png')]
    ...
    def test_annotations_are_created_only_in_debug_mode(self):
        with TemporaryDirectory() as tmp:
            ...
            assert disabled.annotate_points(source, 'capture', 'points', [('p', 5, 6)]) is None
            output = enabled.annotate_regions(source, 'capture', 'regions', ['area'])
            assert output.name == 'capture__regions.png'
