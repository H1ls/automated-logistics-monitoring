...
class TestUiPointResolver:

    def test_context_point_has_priority_over_anchor_and_template(self):
        session = _Session(anchors={'cargo_tab': (30, 40)}, templates={'cargo_tab': 'cargo.png'}, matches={'cargo_tab': (50, 60)})
        ctx = SimpleNamespace(state={'ui_points': {'cargo_tab': {'x': '10', 'y': '20'}}})
        point = UiPointResolver(session).click('cargo_tab', ctx=ctx)
        assert (point.x, point.y, point.source) == (10, 20, 'ctx.state')
        assert session.clicks == [(10, 20)]
        assert session.template_searches == []
    ...
    def test_anchor_is_used_when_context_point_is_missing(self):
        ...
        assert (point.x, point.y, point.source) == (30, 40, 'ui_map.anchor')

    def test_template_is_used_when_context_and_anchor_are_missing(self):
        session = _Session(templates={'cargo_tab': 'cargo.png'}, matches={'cargo_tab': (50, 60)})
        point = UiPointResolver(session).resolve('cargo_tab')
        assert (point.x, point.y, point.source) == (50, 60, 'ui_map.template')

    def test_missing_point_has_contextual_error(self):
        with pytest.raises(RuntimeError, match="UI-точка 'cargo_tab'"):
    ...
    def test_malformed_context_point_is_not_silently_ignored(self):
        ...
        with pytest.raises(RuntimeError, match='Некорректная UI-точка'):
