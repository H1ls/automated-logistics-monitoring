...
class AdminUsersDialogTests(TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_existing_user_username_is_updated_by_id(self):
        ...
        self.assertFalse(dialog.username_edit.isReadOnly())
        dialog.username_edit.setText("new-name")
        dialog.save_user()

        self.assertEqual(client.put_calls[0][0], "/api/v1/users/7")
        self.assertEqual(client.put_calls[0][1]["username"], "new-name")
