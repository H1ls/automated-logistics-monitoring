...
class CaptureRaceUiRetryTests(TestCase):
    def test_retries_missing_anchor_with_fresh_full_screenshot(self):
        ...
        self.assertEqual(artifacts.full_captures, ["capture_race_ui__retry_wait_load_2.png"])
        self.assertEqual(session.sleeps, [0.35])
...
class _Verifier:
    ...
    def verify(self, _ctx, _path):
        ...
        return RaceCardVerification(
            ...
            race_ok=True,
            unit_ok=True,
            expected_race="ВТ000009140",
            expected_plate="О 996 ОР 790",
            recognized_text="race and plate",
        )


class OpenRaceFallbackTests(TestCase):
    @staticmethod
    def _ctx():
        return SimpleNamespace(
            race_name="Рейс (уэ) ВТ000009140 от 19.06.2026",
            meta={"unit": "О 996 ОР 790 SITRAK"},
            state={},
        )
    ...
    def test_uses_ocr_only_when_clipboard_did_not_confirm_race(self):
        ...
        verifier = _Verifier()
        step = OpenRaceStep(
            _OpenSession(clipboard=""),
            errors=None,
            log_func=lambda *_: None,
            artifacts=artifacts,
            race_card_verifier=verifier,
        )

        self.assertTrue(step._is_race_opened(self._ctx()))
        self.assertEqual(verifier.calls, 1)
        self.assertEqual(artifacts.captures, 1)

    def test_clipboard_success_skips_ocr(self):
        artifacts = _OpenArtifacts()
        verifier = _Verifier()
        step = OpenRaceStep(
            _OpenSession(clipboard="ВТ000009140"),
            errors=None,
            log_func=lambda *_: None,
            artifacts=artifacts,
            race_card_verifier=verifier,
        ...
        self.assertEqual(artifacts.captures, 0)
