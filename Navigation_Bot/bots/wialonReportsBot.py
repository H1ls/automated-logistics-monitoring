...
from Navigation_Bot.bots.navi_bot_base import NaviBotBase


class WialonReportsBot(NaviBotBase):
    ...
    def open_reports(self):
        """Открыть “Отчёты”"""
        self._safe_click_xpath(self.sel["reports_tab_xpath"], timeout=10, name="reports_tab")
    ...
    def run_and_wait(self, timeout=45):
        ...
        self._safe_click_xpath(self.sel["run_button_xpath"], timeout=10, name="run_button")
        try:
            self._wait_present_css(self.sel["spinner_waiting_css"], timeout=3)
        except Exception:
            pass

        self._wait_gone_css(self.sel["spinner_waiting_css"], timeout=timeout)
        self._wait_present_css(self.sel["results_table_css"], timeout=10)
    ...
    def extract_times(self, load_zone: str, unload_zone: str) -> dict:
        ...
        for tr in rows:
            ...
            self.log(f"GF raw={gf_raw!r} | norm={gf!r} | "
                     f"load_match={self._zone_match(gf_raw, load_zone)} | "
                     f"unload_match={self._zone_match(gf_raw, unload_zone)}")
